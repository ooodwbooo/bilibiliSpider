...
#  下载视频
def down_video(video_list, title, start_url, page,av):
    num = 1
    ...
    for i in video_list:
        ...
        if len(video_list) > 1:
            ...
            urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'av{}-{}.flv'.format(av,title)),
                                       reporthook=Schedule_cmd)  # 写成mp4也行  title + '-' + num + '.flv'
        num += 1
...
def run(av,SESSDATA):
    ...
    start = av#input('请输入您要下载的B站av号或者视频链接地址:')
    ...
    for item in cid_list:
        ...
        video_list = get_play_list(aid, cid, quality,SESSDATA)
        start_time = time.time()
        down_video(video_list, title, start_url, page ,av)
